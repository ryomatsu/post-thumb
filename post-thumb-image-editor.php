...
class ImageEditor {
  ...
  ########################################################
  function resize($width, $height, $keepratio = true) 
  {
    if(!$this->error) 
    {
      if ($keepratio) {
      	$ratio_x = $this->x / $width;
		$ratio_y = $this->y / $height;
			
		// resize based on width if y is larger
		if ($ratio_y > $ratio_x) $height = $this->y * ($width / $this->x);
		else $width = $this->x * ($height / $this->y);
      }
